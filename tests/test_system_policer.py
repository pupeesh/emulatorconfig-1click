...
def test_system_poller_and_yaml():
    print("--- 1. Testing System Poller Specs ---")
    sys_info = SystemPoller.get_hardware_info()
    dolphin_path, exists = EmulatorResolver.get_emulator_paths("dolphin")
    ...
    print(f"Resolved Dolphin Path: {dolphin_path} (Exists: {exists})\n")
    ...
    # Use ConfigWriter instead of removed YAMLEngine.write_ini_config
    ConfigWriter.write_ini(mock_target, processed_data)
